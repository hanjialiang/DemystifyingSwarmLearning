...
from tensorflow.keras.callbacks import TensorBoard, ModelCheckpoint, ReduceLROnPlateau, EarlyStopping, LearningRateScheduler, LambdaCallback

SLNUM = os.getenv('SLNUM')
SNNUM = os.getenv('SNNUM')
PROBABILITY = float(os.getenv('PROBABILITY', '1'))
...
class ProbabilitySwarmCallback(SwarmCallback):
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        self.current_p = self.sysrand.uniform(0, 1)
        super().on_batch_begin(batch, logs)
    
    def on_epoch_begin(self, batch, logs=None):
        print('on_epoch_begin')
        if self.current_p < self.probability:
            super().on_epoch_begin(batch, logs)
    
    def on_epoch_end(self, batch, logs=None):
        print('on_epoch_end')
        self.probabilities.append(self.current_p)
        if self.current_p < self.probability:
            super().on_epoch_end(batch, logs)
    
    

pSwarmCallback = ProbabilitySwarmCallback(
...
probabilities = pSwarmCallback.probabilities

Path(ospj(OUTPUT_DIR, f'results/{model_name}/{current_time}')).mkdir(parents=True, exist_ok=True)
pickle.dump(
    { 'his': history.history, 'prob': probabilities },
    open(ospj(OUTPUT_DIR, f"results/{model_name}/{current_time}/history_with_prob_" + current_time + ".pkl"), "wb")
    )
