...
def load_data(path):
    global input_shape
    (x_train, y_train), (x_test, y_test) = cifar10_load_data(path)
...
if __name__ == '__main__':
    # load data
    (x_train, y_train), (x_test, y_test) = load_data(DS_PATH)
    model = build_model()
    ...
    if os.getenv('SLNUM') is None:
        LEARNINGTYPE = 'CL'
    else:
        LEARNINGTYPE = 'SL'
    tensorboard = TensorBoard(log_dir=f'./logs/densenet-{LEARNINGTYPE}-{CURRENT_TIME}')
    ...
    if not data_augmentation:
        ...
        pickle.dump(history.history, open(f"{MODEL_NAME}_history.pkl", "wb"))
